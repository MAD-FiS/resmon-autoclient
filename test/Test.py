...
class TestGeneral(unittest.TestCase):
    def test_loadFromSingleMonitorTest(self):
        ...
        monitor = '0.0.0.0'
        measurements = {}
        measurements = x.loadFromSingleMonitor(measurements,
                                               monitor,
                                               rawMeasurements)
        self.assertEqual(
            measurements, {'CPU_USAGE': [[monitor, 'workstation1', 1],
                                         [monitor, 'workstation2', 2]],
                           'RAM_USAGE': [[monitor, 'workstation1', 100]]})

    def test_prepare(self):
        x = View(2)
        monitor = '0.0.0.0'
        measurements = x.prepare({'CPU_USAGE': [[monitor, 'workstation1', 2],
